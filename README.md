...
| Text "Log in"           | N.1 Compare the retrieved text value with the expected text "Log in" to ensure it matches exactly. | The retrieved text value "Log in" matches the expected text "Log in" exactly, indicating that the text is displayed correctly.                                                | yes            |
| Button Cross            | N.2 Click on the cross button and ensure that the login pop up is closed.                          | Closing the Log in window.                                                                                        | yes            |
| Field Email             | N.3 Leave the email field empty and attempt to submit.                                             | Verify that an error message appears stating "Email required."                                                    | yes            |
|                         | N.3.1 Enter an invalid email format (without '@' symbol) and attempt to submit.                    | Verify that an error message appears stating "Email is incorrect."                                                | yes            |
|                         | N.3.2 Enter an email that is not registered with the site and attempt to submit.                   | Verify that an error message appears stating "Wrong email or password."                                           | yes            |
|                         | N.3.3 Verify the visual representation of the email field when an error occurs.                    | Verify that the border color of the email field turns red, and labels associated with the field also turn red.    | yes            |
| Field Password          | N.4 Leave the password field empty and attempt to submit.                                          | Verify that an error message appears stating "Password required."                                                 | yes            |
|                         | N.4.1 Enter an incorrect password for a valid email and attempt to submit.                         | Verify that an error message appears stating "Wrong email or password."                                           | yes            |
|                         | N.4.2 Verify the visual representation of the password field when an error occurs.                 | Verify that the border color of the password field turns red, and labels associated with the field also turn red. | yes            |
| Check-box "Remember me" | N.5 Check the "Remember me" check-box                                                              | Verify that it retains the selection state when the login pop up is closed and reopened.                          | yes            |
</details>
...
| ------------------------ |----------------------------------------------------------------------------------------------------------------------------------------------| ------------------------------------------------------------------------------------------------------------------ | -------------- |
| Elements                 | Action                                                                                                                                       | Expected Result                                                                                                    | State(pass/no) |
| Valid Email and Password | P.1 Enter a valid email and password combination and attempt to submit.                                                                      | Verify that the login is successful and the user is redirected to the appropriate page or dashboard.               | yes            |
| Case Sensitivity         | P.2 Enter the email and password combination with different case sensitivity (e.g., uppercase, lowercase, mixed case) and attempt to submit. | Verify that the login is case-insensitive and the user is able to log in successfully regardless of the case used. | yes            |
</details>
